...
PRODUCTS_RANGE = "Products!A1:W1000"
...
def merge_row_into_product(p, row):
    ...
    # ProductImage = the actual panel/material photo shown as the main image
    # and card thumbnail. LookImage1/LookImage2 = interior design "look book"
    # photos shown further down the product page. If any of these three are
    # filled in on the Products tab, they fully replace the scraped images
    # for this SKU (in that order) — this is how mismatched/wrong images get
    # corrected, and how look-book photos get added per SKU.
    image_cols = [row.get("ProductImage"), row.get("LookImage1"), row.get("LookImage2")]
    ...
    bulk_moq = to_float(row.get("BulkMOQ"))
    bulk_disc = to_float(row.get("BulkDiscountPct"))
    merged["bulk"] = {
        "moq": bulk_moq if bulk_moq is not None else (merged.get("bulk") or {}).get("moq"),
        "discountPct": bulk_disc if bulk_disc is not None else (merged.get("bulk") or {}).get("discountPct"),
    }
